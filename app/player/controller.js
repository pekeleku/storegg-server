...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      if (!res_voucher)
        return res
          .status(404)
          .json({ message: "voucher game tidak tersedia.!" });
      const res_nominal = await Nominal.findOne({ _id: nominal });
      if (!res_nominal)
        return res
          .status(404)
          .json({ message: "nominal game tidak tersedia.!" });
      const res_payment = await Payment.findOne({ _id: payment });
      if (!res_payment)
        return res
          .status(404)
          .json({ message: "payment game tidak tersedia.!" });
      const res_bank = await Bank.findOne({ _id: bank });
      if (!res_bank)
        return res.status(404).json({ message: "bank game tidak tersedia.!" });
      ...
      const payload = {
        historyVoucherTopup: {
          ...
          coinQuantity: res_nominal._doc.cointQuantity,
  ...
  dashboard: async (req, res) => {
    try {
      ...
      const hsitory = await Transaction.find({ player: req.player._id })
        .populate("category")
        .sort({ updatedAt: -1 });
      res.status(200).json({
        data: hsitory,
